...
public class Administrador extends Usuario {
	...
	private String agregarValoresUsuario(Usuario usuario) {
		String entrega = "'" + usuario.getUsuario() + "', '" + usuario.getNombre() + "', '" + usuario.getCedula() + "', '" + usuario.getTipoCedula() + "', " + usuario.getCargo() + ", '"
				+ usuario.getNacionalidad() + "', '" + usuario.getDireccionFisica() + "', '" + usuario.getEmail() + "', '" + usuario.getTelefono() + "', '" + usuario.getCiudad() + "', '"
	...
	public void registrarOficina(Oficina oficina) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String query = "INSERT INTO OFICINA (NOMBRE, DIRECCION, TELEFONO, GERENTE_ID) VALUES (" + agregarValoresOficina(oficina) + ")";
	...
	private String agregarValoresOficina(Oficina oficina) {
		String entrega = "'" + oficina.getNombre() + "', '" + oficina.getDireccion() + "', '" + oficina.getTelefono() + "', '" + oficina.getGerente().getUsuario() + "'";
		return entrega;
	}

	public void registrarPuntoDeAtencion(PuntoDeAtencion puntoDeAtencion) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String query = "INSERT INTO PUNTO_DE_ATENCION (TIPO, LOCALIZACION, OFICINA_ID) VALUES (" + agregarValoresPuntoDeAtencion(puntoDeAtencion) + ")";
		;
	...
	private String agregarValoresPuntoDeAtencion(PuntoDeAtencion puntoDeAtencion) {
		String entrega = "'" + puntoDeAtencion.getTipo() + "', '" + puntoDeAtencion.getLocalizacion() + "', '" + puntoDeAtencion.getOficina().getGerente().getUsuario() + "'";
		return entrega;
	}
	...
	public Administrador obtenerGerente(String gerente) throws SQLException {
		...
		Administrador administrador = conecion.ejecutarQueryObtenerAdministrador("SELECT * FROM USUARIO WHERE USUARIO.USUARIO = '" + gerente + "' AND USUARIO.CARGO = 1");
		return administrador;
	}

	public Oficina obtenerOficina(String stringOficina) throws SQLException {
		ConectionMYSQLAdministradorDAO conecion = new ConectionMYSQLAdministradorDAO();
		Oficina oficina = conecion.ejecutarQueryObtenerOficina("SELECT * FROM OFICINA WHERE OFICINA.GERENTE_ID = '" + stringOficina + "'");
		return oficina;
	}

	public void registrarCuenta(Cuenta cuenta) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String query = "INSERT INTO CUENTA (ID, SALDO, TIPOCUENTA, MONEDA_ID, ESTACERRADA, OFICINA_ID) VALUES (" + agregarValoresCuenta(cuenta) + ")";
		conection.ejecutarQueryRegistrarCuenta(query);
	}

	private String agregarValoresCuenta(Cuenta cuenta) {
		String entrega = "'" + cuenta.getId() + "', " + cuenta.getSaldo() + ", '" + cuenta.getTipoCuenta() + "', '" + cuenta.getMonedaId() + "', '" + cuenta.isEstaCerrado() + "', '" + cuenta.getOficina().getGerente().getUsuario() + "'";
		return entrega;
	}

	public void cerrarCuenta(String idCuenta, String idOficina) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		Cuenta cuenta = conection.ejecutarQueryObtenerCuenta(idCuenta);
		if (cuenta.getOficina().equals(idOficina)) {
			String query = "UPDATE CUENTA SET CUENTA.ESTACERRADA = 'SI' WHERE CUENTA.ID = '" + idCuenta + "' AND CUENTA.SALDO = " + 0;
	...
	public void registrarTransaccionPrestamo(Transaccion transaccion) throws Exception {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		...
		if (registrar) {
			...
			this.actualizarCuentaSaldoAumentar(stringCuentaDestino, transaccion.getValorCuota());
			this.actualizarCuentaSaldoDisminuir(stringCuentaOrigen, transaccion.getValorCuota());
		}
	}

	public Cuenta obtenerCuenta(String idCuenta) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String query = "SELECT * FROM CUENTA WHERE CUENTA.ID = '" + idCuenta + "'";
		Cuenta cuenta = conection.ejecutarQueryObtenerCuenta(query);
		return cuenta;
	}

	private String agregarValoresTransaccion(Transaccion transaccion) {
		String entrega = "'" + transaccion.getId() + "', '" + transaccion.getCantidad() + "', '" + transaccion.getFechaInicio().toString() + "', '" + transaccion.getDone() + "', '"
				+ transaccion.getTipo() + "', '" + transaccion.getCuenta_Origen().getId() + "', '" + transaccion.getCuenta_Destino().getId() + "', '" + transaccion.getCambio() + "', '"
				+ transaccion.getInteres() + "', " + transaccion.getNumeroCuotas() + ", '" + transaccion.getFechaPago().toString() + "', " + transaccion.getValorCuota() + ", "
				+ transaccion.getSaldoPendiente() + "";
		return entrega;
	}

	public void actualizarCuentaSaldoAumentar(String idCuenta, int montoPagado) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		Cuenta cuenta = this.obtenerCuenta(idCuenta);
		int nuevoSaldo = cuenta.getSaldo() + montoPagado;
		String query = "UPDATE CUENTA SET CUENTA.SALDO = " + nuevoSaldo + " WHERE CUENTA.ID = '" + idCuenta + "' AND  CUENTA.ESTACERRADA = 'NO'";
		conection.ejecutarQueryCuentaSaldo(query);
	}

	public void actualizarCuentaSaldoDisminuir(String idCuenta, int montoPagado) throws Exception {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		Cuenta cuenta = this.obtenerCuenta(idCuenta);
		int nuevoSaldo = cuenta.getSaldo() - montoPagado;
		if (nuevoSaldo < 0) {
	...
	public void actualizarTransaccionPrestamo(String idTransaccion, int montoPagado) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		...
		if (nuevoSaldo > 0) {
			String query = "UPDATE TRANSACCION SET TRANSACCION.SALDOPENDIENTE = " + nuevoSaldo + " WHERE TRANSACCION.ID = '" + idTransaccion + "' AND  TRANSACCION.TIPO = 'Prestamo'";
			conection.ejecutarQueryActualizarPrestamo(query);
		}
	}

	public Transaccion obtenerTransaccion(String idTransaccion) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String query = "SELECT * FROM TRANSACCION WHERE TRANSACCION.ID = '" + idTransaccion + "'";
		Transaccion transaccion = conection.ejecutarQueryObtenerTransaccion(query);
		return transaccion;
	}
	...
	public void CerrarPrestamo(String idTransaccion) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String query = "UPDATE TRANSACCION SET TRANSACCION.DONE = 'SI' WHERE TRANSACCION.ID = '" + idTransaccion + "' AND  TRANSACCION.TIPO = 'Prestamo' AND TRANSACCION.SALDOPENDIENTE = " + 0;
	...
	public void registrarUsuarioXCuenta(String usuario_id, String id) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String query = "INSERT INTO USUARIOXCUENTA (USUARIO_ID, CUENTA_ID) VALUES ('" + usuario_id + "', '" + id +"')";
	...
	public void registrarTransaccionDeposito(Transaccion transaccion) throws SQLException {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String stringCuentaDestino = transaccion.getCuenta_Destino().getId();
		boolean registrar = true;
		System.out.println("Busco la cuenta destino");
		...
		if (registrar) {
			String query = "INSERT INTO TRANSACCION (ID, CANTIDAD, FECHAINICIO, DONE, TIPO, CUENTA_ORIGEN, CUENTA_DESTINO, CAMBIO, INTERES, NUMEROCUOTAS, FECHAPAGO, VALORCUOTA, SALDOPENDIENTE) VALUES ("
					+ agregarValoresTransaccion(transaccion) + ")";
			conection.ejecutarQueryRegistrarTransaccion(query);
			this.actualizarCuentaSaldoAumentar(stringCuentaDestino, Integer.parseInt(transaccion.getCantidad()));
		}

	}

	public void registrarTransaccionTransferencia(Transaccion transaccion) throws Exception {
		ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
		String stringCuentaOrigen = transaccion.getCuenta_Origen().getId();
		String stringCuentaDestino = transaccion.getCuenta_Destino().getId();
		boolean registrar = true;
	...
	public ArrayList<Requerimiento3> consultarOperacionMayorMovimiento(Date fechaInicio, Date fechaFinal, Usuario usuarioPrincipal) throws SQLException {
		// TODO Auto-generated method stub
		if (usuarioPrincipal.getCargo() == 1 || usuarioPrincipal.getCargo() == 3) {
			ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
			...
			requerimiento3 = conection.ejecutarQueryConsultarOperacionMayorMovimiento(query);
			return requerimiento3;
		}else {
			throw new SQLException("No se puede hacer esta consulta si no eres gerente general o gerente de oficina");
		}
	}

	public ArrayList<Usuario> consultarUsuarioMasActivo(String tipoOperacion, String tipoBusqueda, int monto, Usuario usuarioPrincipal) throws SQLException {
		// TODO Auto-generated method stub
			ConectionMYSQLAdministradorDAO conection = new ConectionMYSQLAdministradorDAO();
			...
			if (usuarioPrincipal.getCargo() == 3) {
				query += " AND CUENTA.OFICINA_ID = '" + usuarioPrincipal.getUsuario() + "'";
			}
