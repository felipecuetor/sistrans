...
public class RegistroTransaccion extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		Date fechaInicio = new Date(request.getParameter("diaInicio"),
				request.getParameter("mesInicio"),
				request.getParameter("yearInicio"));
		...
		Date fechaPago = new Date(request.getParameter("diaFinal"),
				request.getParameter("mesFinal"),
				request.getParameter("yearFinal"));
		...
		try {
			...
			if (tipo.equals("Prestamo")) {
				Cuenta cuenta_Origen = administrador
						.obtenerCuenta(stringCuenta_Origen);
				Cuenta cuenta_Destino = administrador
						.obtenerCuenta(stringCuenta_Destino);
				transaccion = new Transaccion(id, cantidad, fechaInicio, done,
						tipo, cuenta_Origen, cuenta_Destino, cambio, interes,
						numeroCuotas, fechaPago, valorCuota, saldoPendiente);
				administrador.registrarTransaccionPrestamo(transaccion);
			} else if (tipo.equals("Retiro")) {
				Cuenta cuenta_Origen = administrador
						.obtenerCuenta(stringCuenta_Origen);
				transaccion = new Transaccion(id, cantidad, fechaInicio, done,
						tipo, cuenta_Origen, null, cambio, interes,
						numeroCuotas, fechaPago, valorCuota, saldoPendiente);
				...
				Cuenta cuenta_Destino = administrador
						.obtenerCuenta(stringCuenta_Destino);
				transaccion = new Transaccion(id, cantidad, fechaInicio, done,
						tipo, null, cuenta_Destino, cambio, interes,
						numeroCuotas, fechaPago, valorCuota, saldoPendiente);
			...
			out.println("<p> Fecha De Inicio : "
					+ transaccion.getFechaInicio().toString() + "</p>");
			out.println("<p> Tipo : " + transaccion.getTipo() + "</p>");
			out.println("<p> Cuenta Destino : "
					+ transaccion.getCuenta_Destino().getId() + "</p>");
			out.println("<p> Fecha De Pago : " + transaccion.getFechaPago()
					+ "</p>");
			out.println("<p> Numero De Cuotas : "
					+ transaccion.getNumeroCuotas() + "</p>");
			out.println("<p> Valor De Cuota : " + transaccion.getValorCuota()
					+ "</p>");
			out.println("<p> Saldo Pendiente : "
					+ transaccion.getSaldoPendiente() + "</p>");
			out.println("<br>");
			out.println("</article>");
			out.println("</section>");
			out.println("</section>");
